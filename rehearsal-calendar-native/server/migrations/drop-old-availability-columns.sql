-- Migration: Drop old DATE + TIME columns from native_user_availability
-- Date: December 11, 2025
-- Purpose: Cleanup after migrating to TIMESTAMPTZ columns
-- Requires: migrate-availability-to-timestamptz.sql (must be run first)
...
BEGIN
  ...
  IF EXISTS (
    SELECT 1 FROM native_user_availability
...
BEGIN
  IF EXISTS (
    ...
    RAISE WARNING 'Old columns still exist in native_user_availability table';
  ELSE
    RAISE NOTICE 'Successfully dropped old columns from native_user_availability';
  END IF;
END $$;
