...
/**
 * Check if a user has a password set (for email/password login)
 *
 * @param {number} userId - User ID
 * @returns {Promise<boolean>} True if user has a password
 */
export async function userHasPassword(userId) {
  const user = await db.get(
    'SELECT password_hash FROM native_users WHERE id = $1',
    [userId]
  );

  return user && user.password_hash !== null && user.password_hash !== '';
}

/**
 * Migrate existing email/password user to use auth_providers table
 * This is used for users who registered before OAuth was implemented
 *
 * @param {number} userId - User ID
 * @returns {Promise<void>}
 */
export async function migrateEmailUserToProvider(userId) {
  const now = new Date().toISOString();

  // Check if user already has an 'email' provider entry
  const existing = await db.get(
    'SELECT id FROM native_auth_providers WHERE user_id = $1 AND provider_type = $2',
    [userId, 'email']
  );
  ...
  // Get user's email
  const user = await db.get(
    'SELECT email, last_login_at FROM native_users WHERE id = $1',
    [userId]
  );
  ...
  // Create 'email' provider entry
  await db.run(
    `INSERT INTO native_auth_providers (
      user_id, provider_type, provider_email,
      created_at, updated_at, last_used_at
    ) VALUES ($1, $2, $3, $4, $5, $6)`,
    [userId, 'email', user.email, now, now, user.last_login_at || now]
  );

  logger.info(`[AccountLinking] Migrated user ${userId} to auth_providers table`);
}
