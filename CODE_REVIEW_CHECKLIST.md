...
### [x] 4. Извлечь дублирующийся код AsyncStorage sync ✅
...
### [x] 5. Извлечь дублирующийся код user serialization ✅
...
**Решение:**
Реализован полноценный pull-to-refresh с silent refresh и smart caching:

**CalendarScreen:**
- ✅ Добавлен `RefreshControl` с фиолетовым индикатором
- ✅ Отдельный `refreshing` state (не прячет старый контент)
- ✅ Smart caching в `useRehearsals` (15 секунд)
- ✅ Skeleton только при первой загрузке
- ✅ Фоновое обновление при возврате на экран

**Функционал:**
- Pull-to-refresh (свайп вниз) → принудительное обновление, игнорирует кэш
- Автообновление при фокусе → только если прошло > 15 секунд
- `loading = true` → skeleton (только первая загрузка)
- `refreshing = true` → индикатор pull-to-refresh (старый контент виден)

Улучшает UX: нет "мигания" skeleton'ов, меньше API запросов.
...
**Неделя 1 (Критичное + Важное) - ✅ Завершено:**
1. ✅ SQL injection fix
...
**Неделя 2 (UX/UI) - 5/6 выполнено:**
6. ⏳ Accessibility labels (осталась единственная задача)
7. ✅ Color contrast fix
8. ✅ Haptic feedback
9. ✅ Skeleton screens
10. ✅ Pull-to-refresh + Silent refresh + Smart caching
11. ✅ Glassmorphism
...
**Версия чек-листа:** 1.1

## 🚀 Недавние улучшения

**2026-01-03:**
- ✅ Добавлен pull-to-refresh с RefreshControl
- ✅ Реализован silent refresh (обновление на фоне без skeleton)
- ✅ Smart caching (15 секунд) для уменьшения API запросов
- ✅ Убран DayDetailsModal, long press теперь открывает AddRehearsal с pre-filled датой
- ✅ Улучшен UX: нет "мигания" при переключении вкладок
