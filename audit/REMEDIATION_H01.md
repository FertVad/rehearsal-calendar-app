...
Актуальное дополнение от 18 сентября: [обязательные browser checks, чистая установка и доказательства на commit `c43a0df`](REMEDIATION_H01_FOLLOWUP.md). Ниже сохранён исторический отчёт первого этапа. Его манифест описывает прежний снимок, а не текущий HEAD.
...
- На первом этапе добавлены закреплённая devDependency Playwright, browser regression и тесты реального пути записи/чтения. Тогда обычный backend Jest не требовал браузера; дополнение от 18 сентября включает browser regression в обязательный `npm test`.
